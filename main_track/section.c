...
void section11(uint16_t left_speed, uint16_t right_speed, uint16_t speed) {
    ...
    // Second shortcut
    straight_turn_left_without_tracing(left_speed, right_speed+100, speed);
    ...
    // Third shortcut
    straight_turn_left(left_speed+150, right_speed, speed);
...
void move_until_white(uint16_t left_speed, uint16_t right_speed, uint16_t speed) {
    ...
    while(1) {
        ...
        if(ts1<200 && ts2 < 200 && ts3<200 && ts4<200 && ts5<200 && ts6<200 && ts7<200 && ts8<200) {
            move(0,0);
            P5->OUT &= ~0x08;
            P9->OUT &= ~0x04;
            break;
